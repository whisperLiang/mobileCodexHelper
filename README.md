...
- 支持在手机端直接新建 Codex 线程，创建成功后会同步到电脑端 Codex，会话列表可以马上看到
- 支持在手机端归档旧的 Codex 线程，归档结果会同步到电脑端，避免手机和电脑两边状态不一致
- 新建线程会先创建一个可被桌面 Codex 识别的草稿会话，你从手机发出第一条真实消息后，线程标题和摘要会自动更新
